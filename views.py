...
class UserAPI(MethodView):
    ...
    def access_denided(self, user_id: int) -> Response:
        return jsonify({'result': f'Access denided to notification: {user_id}'})
    ...
    def users_overlap(self, name: str, mail: str) -> Response:
        return jsonify({'result':f'User {name} with e-mail: {mail} already exists'})
    ...
    def get(self, user_id=None) -> Response:
        if user_id:
            ...
            result = {'counts': len(users_name), 'result': users_name}
    ...
    def post(self) -> Response:
        ...
        user_overlap = Users.query.filter(Users.name == name, Users.mail == mail).first()
        if user_overlap:
            return self.users_overlap(name, mail)
        else:
            if pas == c_pas:
                user = Users(name=name, mail=mail, password=pas)
                db.session.add(user)
                db.session.commit()
...
class NoteAPI(MethodView):
    ...
    def autentification(self, owner_name: str, owner_mail: str, owner_pas: str) -> int:
        owner = Users.query.filter(Users.name == owner_name, Users.mail == owner_mail).first()
        if owner:
            if owner_pas == owner.password:
                return owner.id
            else:
                return -1
    ...
    def get(self, note_id=None) -> Response:
        if note_id:
            ...
            result = {'counts': len(note_list), 'result': note_list}
        return jsonify(result)
    ...
    def post(self) -> Response:
        ...
        if owner_id:
            if owner_id != -1:
                ...
                notification = Notifications(title=title, description=description, date=date, owner_id=owner_id)
